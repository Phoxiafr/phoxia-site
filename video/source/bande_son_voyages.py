...
de 9,6 à 49,8 s, impact final sur la signature (59,4 s).

    python3 bande_son_voyages.py   ->  bande-son-voyages.wav (66,6 s, stéréo, 44,1 kHz)
"""
import wave
import numpy as np
...
SR = 44100
DUREE = 66.6
FIN = 59.4  # arrivée de la signature
BPM = 100
TEMPS = 60 / BPM
MESURE = 4 * TEMPS
N = int(SR * DUREE)
...
PAYS_MONTAGE = ['Saint-Martin', 'Saint-Martin', 'Saint-Martin', 'Saint-Martin', 'France', 'France', 'France', 'France', 'France', 'France', 'Var', 'Var', 'Nice', 'Nice', 'Nice', 'Nice', 'Mougins', 'Isola 2000', 'Isola 2000', 'Isola 2000', 'Isola 2000', 'Isola 2000', 'À la montagne', 'À la montagne', 'À la montagne', 'Royaume-Uni', 'Royaume-Uni', 'Italie', 'Italie', 'Italie', 'Sicile', 'Sicile', 'Sicile', 'Andalousie', 'Andalousie', 'Andalousie', 'Andalousie', 'Andalousie', 'Maroc', 'Maroc', 'Maroc', 'Thaïlande', 'Thaïlande', 'New York', 'New York', 'New York', 'Entre amis', 'Entre amis', 'Marseille', 'Marseille']
VIDEOS = {49, 38}  # Marrakech, Vieux-Port : toujours deux temps
d = 9.6
for i, p in enumerate(PAYS_MONTAGE):
    ajoute(declic(), d - (0.3 if i else 0), 0.18, pan=0.3 * (1 if i % 2 else -1))
    d += 1.2 if (i == 0 or PAYS_MONTAGE[i - 1] != p or i in VIDEOS) else 0.6
