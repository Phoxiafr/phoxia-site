...
if (mode === 'apercu') {
  ...
  for (const t of instants) {
    ...
    await scene.screenshot({ path: path.join(ici, 'apercus', `${w}x${h}-${String(t).padStart(5, '0')}.png`) });
  }
...
const sortie = path.join(ici, '..', `phoxia-film-${nom}.mp4`);
const enc = spawn(ffmpeg, [
  '-y', '-f', 'image2pipe', '-framerate', String(fps), '-c:v', 'mjpeg', '-i', '-',
  '-i', path.join(ici, 'bande-son.wav'),
